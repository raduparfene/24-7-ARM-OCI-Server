...
(terraform -chdir=terraform/stacks/base import oci_core_public_ip.reserved ocid1.publicip.oc1.eu-frankfurt-1.amaaaaaaayo4g5 || true) && terraform -chdir=terraform/stacks/base plan
(terraform -chdir=terraform/stacks/base import oci_core_public_ip.reserved ocid1.publicip.oc1.eu-frankfurt-1.amaaaaaaayo4g5 || true) && terraform -chdir=terraform/stacks/base apply -auto-approve
...
```shell
ssh -i ~/.ssh/oci-login-key ubuntu@141.147.2.181
...
```shell
ssh -i ~/.ssh/oci-login-key ubuntu@141.147.2.181 -L 5901:localhost:5901
...
```shell
ssh-keygen -f '~/.ssh/known_hosts' -R '141.147.2.181'
```
